...
public static class Utils
{
    ...
    public static string GetDefaultPassword() { 
        return MD5(DateTime.Now.ToString("yyyyMMdd"),5,10);
    }
    public static string MD5(string pwd, int startIndex, int length)
    {
        MD5 md5 = new MD5CryptoServiceProvider();
        byte[] data = System.Text.Encoding.Default.GetBytes(pwd);
        byte[] md5data = md5.ComputeHash(data);
    ...
    {
        return CutString("", strLen);
    }
    #endregion

    #region
    public static object copy(object soruce, object target)
    {
        Type t1 = soruce.GetType();
        Type t2 = target.GetType();
        if (!t1.FullName.Equals(t2.FullName))
        {
            throw new Exception("复制的对象类型不同，源类型：" + t1.FullName + "目标类型：" + t2.FullName);
        }
        else
        {
            PropertyInfo[] PropertyInfoArray = t1.GetProperties();
            // List<PropertyInfo> prlist = t1.GetProperties().ToList<PropertyInfo>();    
            Array.ForEach<PropertyInfo>(t1.GetProperties(), p =>
            {
                object value = p.GetValue(soruce, null);
    ...
    #endregion
    public static string HtmlParamter(string name, bool onlyOne = true)
    {
        string str = HttpContext.Current.Request.Params[name];
        if (IsEmptyOrNull(str))
            return "";
        else if (onlyOne && str.IndexOf(',') > -1)
