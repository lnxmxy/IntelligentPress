...
namespace RFIDManageSystem
{
    class PrintDGV
    {
        ...
        private static int lbcRowPos;
        private static int lbctmpTop;
        private static int lbcPageNo;
        private static bool lbcNewPage;
        private static int lbcRowsPerPage;
        private static int MergeNum;
        public static void Print_DataGridView(DataGridView dgv1, string tabletext, int num)
        {
            if (dgv1.Rows.Count == 0)
            {
                MessageBox.Show("没有任何数据！", "提示信息");
                return;
            }
            ...
            try
            {
                ...
                foreach (DataGridViewColumn c in dgv.Columns)
                {
                    ...
                    AvailableColumns.Add(c.HeaderText); 
                        int n = 0;
                        if (RFIDApplication.App_Code.ColumnWidth.Column_Name_Width.TryGetValue(c.HeaderText, out n)) { 
                            c.Width = n;
                        } 
                }

                // Showing the PrintOption Form
                PrintOptions dlg = new PrintOptions(AvailableColumns, tabletext);
                ...
                ppvw.Document.DefaultPageSettings.Landscape = true;//横向打印 
                    ...
                    System.Drawing.Printing.PrintEventArgs e)
        {
            try
            {
                ...
                RowPos = 0;
                lbcRowPos = 0;
                //添加页面设置设置(默认)读数据库或读配置文件完成此工作
                DataOperation dao = new DataOperation();
                //页面设置设置(默认)
                dao.ReadPageSettings(printDoc);

                //DataTable dx = new DataTable();
                //dx = DyymbjTable_BLLSub.Get_DyymbjTableAllData();
                //if (dx != null)
                //{
                //    for (int i = 0; i < dx.Rows.Count; i++)
                //    {
                //        if (dx.Rows[i]["PaperDirect"].ToString() == "0")
                //        {
                //            printDoc.DefaultPageSettings.Landscape = true;//设置横向打印
                //        }
                //        else
                //        {
                //            printDoc.DefaultPageSettings.Landscape = false;
                //        }
                //        printDoc.DefaultPageSettings.PaperSize = new System.Drawing.Printing.PaperSize(dx.Rows[i]["PaperSize"].ToString(), int.Parse(dx.Rows[i]["PaperWidth"].ToString()), int.Parse(dx.Rows[i]["PaperHeight"].ToString())); 
                //        printDoc.DefaultPageSettings.Margins.Top = int.Parse(dx.Rows[i]["Margin_Up"].ToString());
                //        printDoc.DefaultPageSettings.Margins.Bottom = int.Parse(dx.Rows[i]["Margin_Down"].ToString());
                //        printDoc.DefaultPageSettings.Margins.Left = int.Parse(dx.Rows[i]["Margin_Left"].ToString());
                //        printDoc.DefaultPageSettings.Margins.Right = int.Parse(dx.Rows[i]["Margin_Right"].ToString());
                    ...
                    System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            int lbctmpTop = e.MarginBounds.Top;
            ...
            try
            {
                ...
                if (PageNo == 1)
                {
                    foreach (DataGridViewColumn GridCol in dgv.Columns)
                    {
                        ...
                        HeaderHeight = (int)(e.Graphics.MeasureString(GridCol.HeaderText,
                                    GridCol.InheritedStyle.Font, tmpWidth).Height) + 35;
                ...
                //计算每页显示行数
                while (lbcRowPos <= dgv.Rows.Count - 1)
                {
                    DataGridViewRow GridRow1 = dgv.Rows[lbcRowPos];
                    if (GridRow1.IsNewRow || (!PrintAllRows && !GridRow1.Selected))
                    {
                        lbcRowPos++;
                        continue;
                    }

                    CellHeight = GridRow1.Height*2;

                    if (lbctmpTop + CellHeight >= e.MarginBounds.Height + e.MarginBounds.Top)
                    {
                        lbcNewPage = true;
                        lbcPageNo++;
                        e.HasMorePages = true;
                    ...
                    else
                    {
                        if (lbcNewPage)
                        {
                            lbctmpTop = e.MarginBounds.Top;
                            lbcNewPage = false;
                            lbctmpTop += HeaderHeight;
                        }
                        lbctmpTop += CellHeight;
                    }

                    lbcRowPos++;
                    if (lbcPageNo == 1) lbcRowsPerPage++;
                }
                ...
                while (RowPos <= dgv.Rows.Count - 1)
                {
                    ...
                    else
                    {
                        ...
                        // Draw Columns Contents 打印时合并单元格
                        switch (MergeNum)
                        {
                            case 4:
                                #region 合并单元格
                                ...
                                foreach (DataGridViewCell Cel in GridRow.Cells)
                                {
                                    ...
                                    #region 合并第一列
                                    if ((Cel.ColumnIndex == 0) && Cel.RowIndex != -1)
                                    {
                                        //当前单元格与下一行单元格内容不相同时，画左竖线、下横线和右竖线
                                        if (Cel.RowIndex < dgv.Rows.Count - 1 && dgv.Rows[Cel.RowIndex + 1].Cells[Cel.ColumnIndex].Value.ToString() != Cel.Value.ToString())
                                        {
                                            e.Graphics.DrawLine(Pens.Black, (int)ColumnLefts[i], tmpTop + CellHeight, (int)ColumnLefts[i] + (int)ColumnWidths[i], tmpTop + CellHeight);
                                            e.Graphics.DrawLine(Pens.Black, (int)ColumnLefts[i] + (int)ColumnWidths[i], tmpTop, (int)ColumnLefts[i] + (int)ColumnWidths[i], tmpTop + CellHeight);
                                            e.Graphics.DrawLine(Pens.Black, (int)ColumnLefts[i], tmpTop, (int)ColumnLefts[i], tmpTop + CellHeight);
                                        }
                                        //当前单元格与下一行单元格内容相同时，画左竖线和右竖线
                                        else
                                        {
                                            e.Graphics.DrawLine(Pens.Black, (int)ColumnLefts[i] + (int)ColumnWidths[i], tmpTop, (int)ColumnLefts[i] + (int)ColumnWidths[i], tmpTop + CellHeight);
                                            e.Graphics.DrawLine(Pens.Black, (int)ColumnLefts[i], tmpTop, (int)ColumnLefts[i], tmpTop + CellHeight);
                                        ...
                                        //当前单元格为每页的最后行时，画下横线
                                        if ((Cel.RowIndex + 1) % lbcRowsPerPage == 0 || Cel.RowIndex == dgv.Rows.Count - 1)
                                        {
                                            e.Graphics.DrawLine(Pens.Black, (int)ColumnLefts[i], tmpTop + CellHeight, (int)ColumnLefts[i] + (int)ColumnWidths[i], tmpTop + CellHeight);
                                        }
                                        //当前单元格与下一行单元格内容相同时，不填写文本
                                        if (Cel.RowIndex > 0 && dgv.Rows[Cel.RowIndex - 1].Cells[Cel.ColumnIndex].Value.ToString() == Cel.Value.ToString())
                                        {
                                            i++;
                                            continue;
                                        }
                                    ...
                                    //#region 根据第一列合并其它列
                                    //else if ((Cel.ColumnIndex == 1 || Cel.ColumnIndex == 2 || Cel.ColumnIndex == 3 || Cel.ColumnIndex == 4) && Cel.RowIndex != -1)
                                    //{
                                    //    //当前单元格与下一行单元格内容不相同时，画下横线和右竖线
                                    //    if (Cel.RowIndex < dgv.Rows.Count - 1 && dgv.Rows[Cel.RowIndex + 1].Cells[0].Value.ToString() != dgv.Rows[Cel.RowIndex].Cells[0].Value.ToString())
                                    //    {
                                    //        e.Graphics.DrawLine(Pens.Black, (int)ColumnLefts[i], tmpTop + CellHeight, (int)ColumnLefts[i] + (int)ColumnWidths[i], tmpTop + CellHeight);
                                    //        e.Graphics.DrawLine(Pens.Black, (int)ColumnLefts[i] + (int)ColumnWidths[i], tmpTop, (int)ColumnLefts[i] + (int)ColumnWidths[i], tmpTop + CellHeight);
                                    //    }
                                    //    //当前单元格与下一行单元格内容相同时，画右竖线
                                    //    else
                                    //    {
                                    //        e.Graphics.DrawLine(Pens.Black, (int)ColumnLefts[i] + (int)ColumnWidths[i], tmpTop, (int)ColumnLefts[i] + (int)ColumnWidths[i], tmpTop + CellHeight);
                                    //    }
                                    ...
                                    //    }
                                    //    //当前单元格与下一行单元格内容相同时，不填写文本
                                    //    if (Cel.RowIndex > 0 && dgv.Rows[Cel.RowIndex - 1].Cells[0].Value.ToString() == dgv.Rows[Cel.RowIndex].Cells[0].Value.ToString())
                                    //    {
                                    //        i++;
                                    //        continue;
                                    //    }
                    ...
                    int RowsPerPage)
        {
            ...
            if (PrintAllRows)
            {
                if (dgv.Rows[dgv.Rows.Count - 1].IsNewRow)
                    cnt = dgv.Rows.Count - 1; // When the DataGridView doesn't allow adding rows
                else
            ...
            e.Graphics.DrawString(PageNum, new Font(dgv.Font, FontStyle.Bold),
                    Brushes.Black, e.MarginBounds.Left + (e.MarginBounds.Width -
                    e.Graphics.MeasureString(PageNum, new Font(dgv.Font,
                    FontStyle.Bold), e.MarginBounds.Width).Width), e.MarginBounds.Top -
                    e.Graphics.MeasureString(PrintTitle, new Font(new Font(dgv.Font,
                    FontStyle.Bold), FontStyle.Bold), e.MarginBounds.Width).Height - 13);
